...
//--------------------------------------------------------------------------------------------------------------------
  ...
  onBeatMarkersChanged: { updateEditMode(false); calculateMarkers() }
...
//--------------------------------------------------------------------------------------------------------------------
  ...
  readonly property int beatsPerDownbeat: 4
  readonly property int beatsPerPhrase: 16

  property var downbeatMarkers
  property var phraseMarkers

  function calculateMarkers() {
    var downbeats = new Array(0)
    var phrases = new Array(0)
    var beatCount = 0
    var gridCount = 0

    var sortedGridMarkers = gridMarkers.slice(0).sort(function comp(a, b) {
      return a - b
    })
    ...
    for (var i = 0; i < beatMarkers.length; i++) {
      ...
      if (marker >= sortedGridMarkers[gridCount]) {
        // New grid marker. Start counting.
        beatCount = 0
        gridCount++
      }

      if (gridCount > 0) {
        if (beatCount % beatsPerPhrase == 0) {
          phrases.push(marker)
        } else if (beatCount % beatsPerDownbeat == 0) {
